...
import { profile, subscriptions } from "../RouteConstants";
...
export default function UserBillModal({ profileData }) {
			...
			axios
				...
				.then((response) => {
					if (response.status === 200) {
						...
						setStateList((prev) =>
							[...prev].concat(response.data.Data.stateList)
						);
					}
				})
				.catch((e) => {
	...
	const handleSaveBillingAddr = () => {
		...
		axios
			...
			.then((result) => {
				if (result.data.status === 200) navigate(subscriptions);
				setBillingDetails((prev) => ({
					...prev,
					stateName: null,
					stateCode: undefined,
	...
	return (
		<div>
			...
			<div
				...
				tabIndex='-1'
				role='dialog'
				aria-labelledby='StateCheckModalLabel'
				aria-hidden='true'
			>
						...
						<div className='modal-body'>
							<div>
								...
								profileData.state !== null ? (
									...
									<strong>
										You haven't set your state yet
									</strong>
								)}
