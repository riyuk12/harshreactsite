...
function Forgot_Password() {
  ...
  const handleNext = async () => {
    if (step === 1) {
      try {
        setStep(2);
        const response = await axios.post("YOUR_FORGOT_PASSWORD_API_ENDPOINT", {
          email,
        });
  ...
  return (
    ...
    >
      <Card
        style={{ width: "700px", padding: "30px", height: "500px" }}
        className="cards-container"
      >
        <h4 className="Forgot_Password_head text-center">
                ...
                {[1, 2, 3, 4, 5, 6].map((_, index) => (
                  <input
                    ...
                    style={{
                      ...
                      border: "2px solid #71269C", // Add border
